    ...
    return Scaffold(
      ...
      body: ResponsiveCenter(
        ...
        child: StreamBuilder<List<UserProfile>>(
          stream: userService.getMaintainers(_organizationId!),
          builder: (context, userSnapshot) {
            ...
            return StreamBuilder<List<Report>>(
              ...
              builder: (context, reportSnapshot) {
                ...
                // Reusable Map for grouping
                final groupedReports = <String?, List<Report>>{};
                for (var report in allReports) {
                  groupedReports.putIfAbsent(report.assignedTo, () => []).add(report);
                }

                return ListView.builder(
                  padding: const EdgeInsets.all(16),
                  itemCount: maintainers.length + (groupedReports.containsKey(null) ? 1 : 0),
                  itemBuilder: (context, index) {
                    if (index < maintainers.length) {
                      final maintainer = maintainers[index];
                      final reports = groupedReports[maintainer.uid] ?? [];
                      return _MaintainerGroup(
                        title: maintainer.displayName,
                        reports: reports,
...
class _MaintainerGroup extends StatelessWidget {
  final String title;
      ...
      children: [
        Padding(
          ...
          child: Row(
            children: [
              Icon(
                isUnassigned ? Icons.help_outline_rounded : Icons.person_rounded,
                size: 20,
                color: isUnassigned ? colorScheme.error : colorScheme.primary,
              ),
